...
namespace Sportland.Sports.Dodgeball
{
    ...
    public class Ball : MonoBehaviour
    {
        ...
        [SerializeField] private float lobApex = 0.5f;   // near-flat base; distance/clearance add the height
        ...
        [SerializeField] private float lobApexPerUnit = 0.15f;
        [Tooltip("Lob apex floor (u) when an opponent stands in the pass lane. Must clear a jumping defender's reach (PickupHeightFor ≈ pickupMaxHeight + jumpHeight = ~3.0 u), so 3.5+ keeps the ball above their grasp at the midpoint of the arc.")]
        [SerializeField] private float lobClearanceApex = 3.5f;
        [Tooltip("Maximum lob apex (u) after scaling. Capped here so very long passes don't moonball off-screen, but still tall enough that the descent stays above defender reach for most of the path.")]
