...
> Code-agnostic by design. Companion to `HubActionsDesign.md` (days as currency — this doc defines the year those days live in), `HubWorldDesign.md` (the "join a league" intro step), and `ConflictChemistryDesign.md` (season boundaries as ego checkpoints).
...
- **The overnight tick owns transitions.** Day advancement (from `HubActionsDesign.md` §2) also processes season-phase boundaries: publishing fixtures, closing seasons, running promotion/relegation, opening offseasons.
